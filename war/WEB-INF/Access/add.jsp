...
<body>
	...
	<div class="div2" align="center">
		<form name="miFormulario" class="form" method="get" action="/access/add">
		...
		<%Role e= (Role) roles.get(i);%>
			<%if(e.getRoleStatus()==true){%>
			<option value="<%=e.getRoleName()%>"><%=e.getRoleName()%></option>	
			<%}%>
		<%}%>
		</select>
		<br><br>
		<label for="resource" class="form-label">Resources:</label><br>
		<select name="resource" id="resource">
		<%for (int i = 0; i < resources.size(); i++) {%>
		<%Resource e= (Resource) resources.get(i);%>
			<%if(e.getResourceStatus()==true){%>
				<option value="<%=e.getResourceName()%>"><%=e.getResourceName()%></option>
			<%}%>		
			
		<%}%>
		</select>
		<br><br>
		<input type="submit" class="btn-submit" value="Create access">
		<br><br>
	</form>
	</div>
</body>
