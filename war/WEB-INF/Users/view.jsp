...
<div class="div2" align="center">
	<table class="table1">
		...
		<%Users e= (Users) users.get(i);%>
		<tr>
			<td> <%=e.getId()%></td>
			<td> <%=e.getUserMail()%></td>
			<td> <%=e.getUserRole()%></td>
			<td> <%=e.getUserStatus()%></td>
			<td> <%=e.getUserCreated()%></td>
			<%if(e.getUserMail().equals("laqui2606@gmail.com")){ %>
				<td><a href="/user/view?id=<%=e.getId()%>">view</a></td>
			<%}else if(e.getUserMail().equals("richarteq@gmail.com")){ %>
			<td> 
				<a href="/user/view?id=<%=e.getId()%>">view</a>
			 	<a href="/user/update?id=<%=e.getId()%>">edit</a>
			</td>
			<%}else{%>
			<td> 
				<a href="/user/view?id=<%=e.getId()%>">view</a>
			 	<a href="/user/update?id=<%=e.getId()%>">edit</a>
			 	<a href="/user/delete?id=<%=e.getId()%>">delete</a>
			</td>
			<%} %>
		</tr>
		<%}%>
