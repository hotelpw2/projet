	...
	<script>
		function validar(){
			if (/^[A-Za-z][A-Za-z]*/.test(document.miFormulario.name.value) == false) {
				alert("The name of role  entered is not valid.");
				return false;
			}
		}
	</script>
<body>
	...
	<div class="div2" align="center">
		<form name="miFormulario" class="form" method="get" action="/roles/edit" onsubmit="return validar();">
		<input type="hidden" name="action" value="rolesCreateDo"/>
		
		<label for="name" class="form-label">Role name:</label>
		<input type="text" id="name" name="name" class="form-input" value="<%=e.getRoleName()%>" required>
		<br>
		<br>
		<label for="status" class="form-label">Event Status: <colspan>(<%=e.getRoleStatus()%>)</colspan></label><br>
		<select name="status" id="status">
  		...
  		<br><br>
		<input type="hidden" id="roleId2" name="roleId2" class="form-input" value="<%=e.getId()%>">
		<input type="submit" class="btn-submit" value="Update role">
