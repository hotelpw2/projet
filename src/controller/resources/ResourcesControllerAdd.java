	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException{
		...
		//Cuando hay alguien logeado
			...
			List<Resource> resources = (List<Resource>) pm.newQuery(query).execute();
				if(req.getParameter("action").equals("resourcesCreate")) {
					RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Resources/add.jsp");
					dispatcher.forward(req, res);
				} else if(req.getParameter("action").equals("resourcesCreateDo")) {
						for(int i=0;i<resources.size();i++) {
							if(resources.get(i).getResourceName().equals(rec)) {
								existe=true;
							}
						}
						if(existe) {
							...
							Resource a = new Resource((String)req.getParameter("name"),true,sdf.format(date));
							try{
								pm.makePersistent(a);
							}finally{
								pm.close();
