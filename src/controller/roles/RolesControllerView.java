...
public class RolesControllerView extends HttpServlet{
	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException {
		...
		//Cuando hay alguien logeado
			...
			RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Roles/view.jsp");
			try {
			...
			*/
			...
			String rolePersonaEncontrada = null;
			...
			for(int i=0;i<users.size();i++) {
				...
				if(tmp.equals(emailCurrent)) {
					...
					rolePersonaEncontrada=users.get(i).getUserRole();
			...
			for(int i=0;i<access.size();i++) {
				...
				if(tmp.equals(rolePersonaEncontrada)) {
