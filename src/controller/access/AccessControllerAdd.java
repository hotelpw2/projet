	...
	public void doGet(HttpServletRequest req, HttpServletResponse res) throws IOException, ServletException{
		...
		//Cuando hay alguien logeado
			...
			List<Resource> resources = (List<Resource>) pm.newQuery(query).execute();
				if(req.getParameter("action").equals("accessCreate")) {
					req.setAttribute("roles", roles);
					req.setAttribute("resources", resources);
					RequestDispatcher dispatcher = getServletContext().getRequestDispatcher("/WEB-INF/Access/add.jsp");
					dispatcher.forward(req, res);
				} else
					if(req.getParameter("action").equals("accessCreateDo")) {
					// create the new user
					Access a = new Access((String)req.getParameter("role"),(String)req.getParameter("resource"),true,sdf.format(date));
				try{
					pm.makePersistent(a);
				}finally{
					pm.close();
			...
			if(userExisteBaseDatos && estadoUser && estadoAcceso&& rolEnBaseDatos) {
						...
						if(accesoRolePersonaEncontrada.equals(s1)) {
							boolean existeRoleAccess=false;
							Date date = new Date();
							SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");
							PersistenceManager pm = PMF.get().getPersistenceManager();
							String query = "select from " + Resource.class.getName();
							...
							List<Resource> resources = (List<Resource>) pm.newQuery(query).execute();
								if(req.getParameter("action").equals("accessCreate")) {
									...
									if(req.getParameter("action").equals("accessCreateDo")) {
										for(int i=0 ;i<access.size();i++) {
											if(req.getParameter("role").equals(access.get(i).getRoleAccess())) {
												System.out.println("ya existe un accesso para este role");
												existeRoleAccess = true;
											}
										}
										if(existeRoleAccess) {
											res.sendRedirect("/access");
										}else if (!existeRoleAccess) {
											// create the new user
											Access a = new Access((String)req.getParameter("role"),(String)req.getParameter("resource"),true,sdf.format(date));
										try{
											pm3.makePersistent(a);
										}finally{
											pm3.close();
										}
										}
